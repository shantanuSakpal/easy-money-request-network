const InvoiceTemplate = ({ businessName, businessAddress, businessContact, businessEmail, note, recipient }) => {
  return (
    <div className="w-3/4 mx-auto p-8 bg-white">
      ...
      <div className="flex justify-between items-start mb-8">
        <div>
          ...
          <p className="text-gray-600">{businessEmail} - {businessContact}</p>
        </div>
      ...
      <div className="mb-8 border-b pb-4">
        <h3 className="text-gray-700 font-semibold mb-2">Employee Details:</h3>
        <div className="flex justify-between gap-4">
          <div>
            <p className="text-gray-600">Name: {recipient.name}</p>
            <p className="text-gray-600 text-left">Department/Team: {recipient.teamName}</p>
          </div>
          <div className="">
            <p className="text-gray-600 text-right">Email: {recipient.email}</p>
            <p className="text-gray-600 text-right">Billed at: {recipient.country} - {recipient.postalCode}</p>
          </div>
        </div>
        <p className="text-gray-600 ">Wallet Address: {recipient.walletAddress}</p>
      </div>
      ...
      <div className="mb-8 border-b pb-4 ">
        ...
        <a className=" text-blue-500" href="LINK">Request Scan - LINK </a>
      </div>
        ...
        <table className="w-full">
          ...
          <tbody>
            ...
            <tr className="border-b">
              ...
              <td className="text-right p-2">-0.001 eth</td>
            </tr>
            <tr className="font-bold">
              <td className="p-2">Net Pay</td>
              <td className="text-right p-2">{(recipient.amount - 0.001).toFixed(4)} eth</td>
            </tr>
          </tbody>
        </table>
      </div>
